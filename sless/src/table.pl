...
my $max_width = 80;
my $option_col_number = 1;
my $option_record_number = 1;
...
while (my $line = <STDIN>) {
    ...
    if ($record_count == $max_line_count) {
        printRecord(shift(@records), $col_lengths); # カラム番号
        printRecord(shift(@records), $col_lengths, 1); # カラム名
